            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">车票展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="chepiaoName" id="chepiaoName" placeholder="班车" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../chepiao/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.chepiaoPhoto?item.chepiaoPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../chepiao/detail.jsp?id='+item.id)"><span>{{item.chepiaoName}}</span></div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 chepiaoContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chepiaoContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#chepiaoName').val() != null && jquery('#chepiaoName').val() != "" && jquery('#chepiaoName').val() != "null" ) {
                param['chepiaoName'] = jquery('#chepiaoName').val();
            }
     
            var chepiaoTypes = $("#chepiaoTypes").find("option:selected").val();
            if (chepiaoTypes != null && chepiaoTypes != "" && chepiaoTypes != "null") {
                param['chepiaoTypes'] = chepiaoTypes;
            }
                                    // 获取列表数据
            http.request('chepiao/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function chepiaoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chepiao_types", 'get', {}, function (res) {
               if(res.code == 0){
